# prend en parametre deux argument le nom du fichier a extraire  la destination des fichier a creer et leur noms
sub ExtractionDesFichiers($$$)
{
   my ($FileName,$Path,$nom)=@_; # Tableau des paramètres 
   ...
   while(!eof(F)){
     if($str =~m /\.I\s/){ # On regarde si s$tr contient la chaîne .I
        ...
        $Num=$str;
        print COL "$nom-$Num\n";
        print "Processing ... $nom-$Num\n";
        open(NF,">$Path/$nom-$Num");
     }
...
ExtractionDesFichiers(@ARGV[0],@ARGV[1],@ARGV[2]);
